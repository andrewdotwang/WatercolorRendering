...
Ray Camera::generate_ray(double x, double y) const {
  ...
  float vFov_rad = vFov * PI / 180.0;

  float horizontal = tan(0.5 * hFov_rad);
  float vertical = tan(0.5 * vFov_rad);

  float shifted_x = (2.0 * x) - 1.0;
  float shifted_y = (2.0 * y) - 1.0;


  Vector3D cspace(shifted_x * horizontal, shifted_y * vertical, -1);
  Vector3D worldspace = c2w * cspace;
  worldspace.normalize();


  Ray ret(pos, worldspace, fClip);
  ret.min_t = nClip;
  return ret;
}
